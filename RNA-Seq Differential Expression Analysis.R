...
hbr1 <- as.data.frame(fread("/ihome/hugen2071-2024f/laa196/project7/HBR_Rep1ReadsPerGene.out.tab", skip=4))
hbr2 <- as.data.frame(fread("/ihome/hugen2071-2024f/laa196/project7/HBR_Rep2ReadsPerGene.out.tab", skip=4))
uhr1 <- as.data.frame(fread("/ihome/hugen2071-2024f/laa196/project7/UHR_Rep1ReadsPerGene.out.tab", skip=4))
uhr2 <- as.data.frame(fread("/ihome/hugen2071-2024f/laa196/project7/UHR_Rep2ReadsPerGene.out.tab", skip=4))
...
d
png("MDS_plot.png", width=800, height=800)
...
d1 <- estimateCommonDisp(d, verbose=T)
d1 <- estimateTagwiseDisp(d1)
png("BCV_plot1.png", width=800, height=800)
plotBCV(d1)
dev.off()
...
png("BCV_plot2.png", width=800, height=800)
plotBCV(d2)
dev.off()
...
#To get the gene names when the differential expression is not 0 (not significant)
de_genes <- rownames(de1)[de1 != 0]
write.table(de_genes, file="DE_genes_HBR_vs_UHR.txt", 
            quote=FALSE, row.names=FALSE, col.names=FALSE)
#To get the all gene names
All_de_genes <- rownames(de1)
write.table(All_de_genes, file="All_DE_genes_HBR_vs_UHR.txt", 
            quote=FALSE, row.names=FALSE, col.names=FALSE)

# Plot log(fold change) vs. log(cpm)
# Significantly differently-expressed genes are shown in red
de1tags12 <- rownames(d1)[as.logical(de1)]
png("Smear_plot.png", width=800, height=800)
